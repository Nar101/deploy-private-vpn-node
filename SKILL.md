...
description: >
  为个人合法使用购买、部署、复制、验收、维护或排查自用 VPS 代理节点，重点支持 Ubuntu LTS、VLESS + Reality + Vision、Xray/3X-UI、腾讯云轻量应用服务器和 Shadowrocket。用户提到私人 VPN、个人代理、干净 IP、自建节点、VPS 节点、Reality、3X-UI、Shadowrocket 超时、节点复制、换 IP、节点维护或希望 AI 代为完成部署时使用。仅处理少量自用节点，不用于机场、多用户售卖、公开订阅或规避服务条款。
---

# 部署私人 VPN 节点

把“买一台海外 VPS”交付成一个可验证、可维护、凭证不外泄的个人代理节点。不要把提示词、安装成功或单次延迟当成交付完成。
...
开始时报告模式、当前已知事实、需要用户判断的购买项和完成标准。用户不懂技术时，给一个推荐项和理由，不把协议细节变成用户必须做的选择。
...
- 付款、实名、验证码、密码创建和最终安全授权由用户完成或在动作发生前确认。
...
当前 `v0.2` 来自一次已完成的腾讯云东京部署和一次同实例零断流安全加固：Ubuntu 24.04、3X-UI、Xray、VLESS Reality Vision、Shadowrocket。腾讯云路径已真实验证；其他供应商、其他客户端和自动订阅仍需在使用时重新验证，不得直接宣称通用稳定。
