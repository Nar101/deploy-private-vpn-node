...
description: >
  帮助不懂服务器的新手从零搭建自用 VPN/代理节点：先选购一台海外 VPS，再由 AI 完成 Ubuntu、VLESS Reality、Xray/3X-UI 和 Shadowrocket 接入，直到手机或电脑真实可用。用户说不想继续买共享梯子或机场订阅、想拥有自己的节点、第一次买 VPS、想让 AI 代为搭建，或后续需要复制、排障和维护节点时使用。额外覆盖凭证保护、双层防火墙、独立验收、备份和零断流维护。仅处理个人或极少数受信任设备的合法使用，不用于机场、多用户售卖、公开订阅或规避服务条款。
---
...
当前 `v0.2.1` 来自一次已完成的腾讯云东京新手部署和一次同实例零断流安全加固：Ubuntu 24.04、3X-UI、Xray、VLESS Reality Vision、Shadowrocket。腾讯云路径已真实验证；其他供应商、其他客户端和自动订阅仍需在使用时重新验证，不得直接宣称通用稳定。
